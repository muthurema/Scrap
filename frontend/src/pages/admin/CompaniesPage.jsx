...
export default function CompaniesPage() {
  ...
  const [form, setForm] = useState({ name: "", default_jurisdiction: "" });
  ...
  const create = async (e) => {
    ...
    try {
      ...
      setForm({ name: "", default_jurisdiction: "" });
      setOpen(false);
      load();
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-md w-[calc(100%-2rem)] rounded-sm">
            ...
            <form onSubmit={create} className="space-y-4">
              ...
              <Button type="submit" disabled={busy} data-testid="submit-company-btn" className="w-full rounded-sm bg-emerald-600 hover:bg-emerald-700 text-white h-10 font-semibold tracking-tight">
      ...
      <div className="border border-slate-300 bg-white overflow-x-auto">
        <table className="w-full text-sm min-w-[560px]">
          <thead className="bg-slate-100">
            <tr>
              {["Name", "Default juris.", "Created", "Status"].map((h) => (
          ...
          <tbody>
            {loading && <tr><td colSpan={4} className="px-3 py-6 text-center font-mono text-xs uppercase tracking-wider text-slate-500">LOADING…</td></tr>}
            {!loading && items.length === 0 && (
              <tr><td colSpan={4} className="px-3 py-8 text-center text-sm text-slate-500">
